# AGENTS.md - AI Agent Development Guide for tap-concur
...
This tap follows the Singer specification and uses the Hotglue Singer SDK (hotglue_singer_sdk) to extract data from Concur.

### Key Components

1. **Tap Class** (`tap_concur/tap.py`): Main entry point, defines streams and configuration
2. **Client** (`tap_concur/client.py`): HTTP client, stream base class, and (where applicable) how auth is attached to requests
3. **Streams** (`tap_concur/streams.py`): Stream classes, schemas (`th.PropertiesList`), and—depending on stream type—`path`, `query`, or custom extraction hooks
...
1. Add a stream class in `tap_concur/streams.py` (follow naming: PascalCase + `Stream`, Singer **`name`** in snake_case—consistent with existing streams).
...
5. Add the class to **`STREAM_TYPES`** in `tap.py` and to the **`from tap_concur.streams import (...)`** block (Cookiecutter pre-fills both from **`stream_names`**).
...
class MyNewStream(ConcurStream):
    name = "my_new_stream"
...
The agent should align **`name`**, **`path`** (or **`query`** for GraphQL), **`schema`**, **`records_jsonpath`** / **`get_next_page_token()`** in **`tap_concur/client.py`** when needed, and **`STREAM_TYPES`** + imports in **`tap.py`** for new streams. See **Handling Pagination** and **Adding a New Stream** above.
...
```bash
tap-concur --config config.json --discover
tap-concur --config config.json --catalog catalog.json
```

### Keeping tap config, docs, and env in sync

Authoritative settings live in **`config_jsonschema`** on the tap class in `tap_concur/tap.py` (Hotglue Singer SDK). Everything operators and Hotglue jobs read should match that schema.
...
1. Update `config_jsonschema` in `tap_concur/tap.py`.
2. Update **`README.md`**: the configuration table, the example `config.json`, and any prose that names settings.
3. Update **`.env.example`** so variable names stay aligned with `tap-concur --about` / `--config=ENV` (keys are derived from config property names).
...
TAP_CONCUR_API_URL=https://api.example.com
TAP_CONCUR_ACCESS_KEY=your_access_key_here
TAP_CONCUR_BATCH_SIZE=100
```
...
- Prefer `tap-concur --about` (or `--format=markdown`) as the generated reference for your tree.
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`tap-concur --version` with your venv activated), Python version, and a minimal reproduction case when reporting bugs.
